...
suite('Extension Test Suite', () => {
	test('Document symbols', async () => {
		...
		const extension = vscode.extensions.getExtension('anticultist.ms-access-dump-format') as vscode.Extension<any>;
		await extension.activate();
		assert.strictEqual(extension.isActive, true);

		await timeout(1500);  // TODO: replace this

		const symbols: vscode.DocumentSymbol[] = await vscode.commands.executeCommand("vscode.executeDocumentSymbolProvider", docUri);
...
function getExampleFileDocUri(p: string) {
	return vscode.Uri.file(path.resolve(__dirname, "../../../tree-sitter-ms-access-dump/examples", p));
}
