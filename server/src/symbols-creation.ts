...
function scanAssignment(uri: string, assignment_node: Parser.SyntaxNode, symbols: SymbolInformation[]) {
    ...
    if (node.firstNamedChild?.type == "identifier") {
        if (node.firstNamedChild.text == "Form" || node.firstNamedChild.text == "Report") {
            symbols.push({
                ...
                kind: SymbolKind.Struct,
                location: Location.create(uri, Range.create(Position.create(node.startPosition.row, node.startPosition.column),
